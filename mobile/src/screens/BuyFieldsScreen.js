...
export default function BuyFieldsScreen() {
  ...
  const search = async (location = pin) => {
    if (!location) { setMessage('Set your location first (GPS or tap the map).'); return; }
    try {
      const res = await api.get(`/parcels/nearby/?lat=${location.lat}&lng=${location.lng}&radius=${radius}`);
  ...
  const renderHeader = () => (
    <View>
      ...
      <TouchableOpacity style={[styles.button, styles.searchButton]} onPress={() => search()}>
        <Text style={styles.buttonText}>Find fields</Text>
      </TouchableOpacity>
      ...
      <Text style={styles.resultsHeading}>Fields ({visibleFields.length})</Text>
    </View>
  );
        ...
        <View style={styles.card}>
          <View style={styles.cardHeaderRow}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.cardSubtitle}>{f.opg_name}</Text>
              {f.owner_phone ? <Text style={styles.phoneText}>📞 {f.owner_phone}</Text> : null}
            </View>
            {f.owner_phone ? (
              <TouchableOpacity style={styles.callButton} onPress={() => Linking.openURL(`tel:${f.owner_phone}`)}>
